    ...
    public static class DownloadUrlResolver
    {
        ...
        public static IEnumerable<VideoInfo> GetDownloadUrls(string videoUrl, bool decryptSignature = true)
        {
            ...
            try
            {
                ...
                string videoThumb = GetVideoThumbnail(json);
                string videoAuthor = GetAuthor(json);
                double videoLength = GetVideoLength(json);
        ...
        private static string GetVideoThumbnail(JObject json)
        {
            JToken thumb = json["args"]["thumbnail_url"];

            string ret = thumb == null ? String.Empty : thumb.ToString();
            if (!string.IsNullOrWhiteSpace(ret))
            {
                ret = ret.Replace("default", "hqdefault");
            }

            return ret;
        }

        private static string GetAuthor(JObject json)
        {
            JToken author = json["args"]["author"];

            return author == null ? String.Empty : author.ToString();
        }

        private static double GetVideoLength(JObject json)
        {
            JToken author = json["args"]["length_seconds"];

            return Convert.ToDouble(author == null ? String.Empty : author.ToString());
        }
        ...
        private static void ThrowYoutubeParseException(Exception innerException, string videoUrl)
        {
            throw new YoutubeParseException("Could not parse the Youtube page for URL " + videoUrl + "\n" +
                                            ...
                                            "Please report this bug at www.github.com/flagbug/Symphony.Player.Youtube/issues", innerException);
        }
