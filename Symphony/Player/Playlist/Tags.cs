...
namespace Symphony.Player
{
    ...
    {
        //STATIC FIELDS
        private static PlayerLogger log = new PlayerLogger("Tag");
        ...
        public List<TagImage> Pictures { get; private set; }
        ...
        public void SetTarget(VideoInfo videoInfo)
        {
            Type = PlaylistItemType.Youtube;

            VideoInfo = videoInfo;

            FilePath = YoutubeItem.GetFilePathFromVideoInfo(videoInfo);

            GetYoutubeTag(this, videoInfo);

            Codec = GetCodecMessage(videoInfo.VideoExtension);
        }
        ...
        private static async void GetYoutubeTag(Tags tag, VideoInfo vi)
        {
            tag.Title = vi.Title;
            ...
            if (vi.ThumbnailUri != null)
            {
                ...
                Task t = Util.LimitedTaskScheduler.Factory.StartNew(delegate
                {
                    ...
                    if (buffer != null)
                    {
                        tag.Pictures.Add(new TagImage(buffer));

                        tag.TagUpdated?.Invoke(tag, null);
                    }
                });
        ...
        private static byte[] DownloadData(string uri, int retry)
        {
            if(uri == null || uri == string.Empty || retry >= 5)
            {
                return null;
            }
            ...
            {
                Logger.Error("Error to download data from : " + uri + "\n" + ex.ToString());

                Thread.Sleep(500);

                return DownloadData(uri, retry+1);
            }
        }

        private static void GetId3Tag(Tags tag)
        {
            try
            {
                using (TagLib.File tagfile = TagLib.File.Create(tag.FilePath))
                {
                    if (tagfile.Tag != null)
                    {
                        ...
                        if (tagfile.Tag.Pictures != null)
                        {
                            ...
                            {
                                tag.Pictures.Add(new TagImage(tagfile.Tag.Pictures[i].Data.Data));
                            }
                        }
                        ...
                        for (int i = 0; i < AlbumArtFolders.Length && UseLocalAlbumArts; i++)
                        {
                            ...
                            try
                            {
                                if (current.EndsWith("/") || current.EndsWith("\\"))
                                {
                                    ...
                                    if (Directory.Exists(folderPath))
                                    {
                                        ...
                                        foreach (FileInfo fi in fiList)
                                        {
                                            ...
                                            {
                                                tag.Pictures.Add(new TagImage(fi.FullName));
                                            }
                                        }
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        tag.Pictures.Add(new TagImage(path));
                                    }
                                }
                            }
                            catch
        ...
        public bool CanEditAlbumArt()
        {
            if(Type != PlaylistItemType.File)
            {
                return false;
            }
            else
            {
                ...
                if (ext.EndsWith("wav") || ext.EndsWith("wave") || ext.EndsWith("pcm"))
                {
                    return false;
                }
        ...
        public static void FixTag(Tags tag)
        {
        ...
        public static void NullTag(Tags tag)
        {
            tag.Artist = "";
            tag.Album = "";
            tag.Title = "";
